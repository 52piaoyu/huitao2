...
class DuibaController extends AppController
{
    ...
    {
        //uid预扣款
        if (!$this->dparam['user_id'] || !$this->dparam['price']) {
            info('请求信息不完整', -1);
        }
        ...
        if (!empty($p_info) && $p_info['totle'] >= 500) {
            info('超过本周提现限额请您去找客服进行提现', -1);
        }
        ...
        if ($uid_info['price'] < $this->dparam['price']) {
            info('余额不足', -1);
        }
        if (empty($this->dparam['alipay_num']) || empty($this->dparam['alipay_name'])) {
            info('支付宝信息不完整', -1);
        }
        ...
        M()->startTrans();
        try {
            if (!A('Uid:updateUid', [$uid_where,$uid_data])) {
                E('申请提现失败1');
            }
            if (!A('Pnow:addPnow', [$pnow_data])) {
                E('申请提现失败2');
            }
            //通知用户有提现消息
    ...
    {
        if (empty($_POST['uid']) || empty($_POST['pid']) || empty($_POST['price']) || empty($_POST['alipay']) || empty($_POST['alipay_name'])) {
            info('参数不全', -1);
        }
    ...
    public function feedback()
    {
        ...
        if (empty($p_info['duiba_etime'])) {
            ...
            M()->startTrans();
            try {
                if (!A('Pnow:updatePnow', [$p_where,$p_data])) {
                    E('修改提现表失败');
                }
                if (!A('Uid:updateUid', [$uid_where,$uid_data])) {
                    E('申请提现失败1');
