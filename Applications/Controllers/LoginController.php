...
class LoginController extends AppController
{
    ...
    public function login()
    {
        ...
        try {
            ...
            if (!empty($this->dparam['token'])) {    //检查免登入 token 是否和库里一致
                $this->checkToken();
            } elseif (!empty($this->dparam['id_code'])) {    //存在验证码码表示修改密码登入
                $this->checkCode(2);
            } else {
                $this->checkPwd();
            }    //检查密码是否一致
    ...
    public function checkUid($type)
    {
        ...
        if (empty($this->uid_info) && $type == 2) {
            info('请先注册!', -1);
        }
        if (!empty($this->uid_info) && $type == 1) {
            info('您已是惠淘会员,请登入!', -1);
        }
    ...
    public function checkCode($type)
    {
        ...
        $info    =    M()->query($sql, 'single');
        if (empty($info['vaild_code']) || $info['vaild_code'] != $this->dparam['id_code']) {
            info('验证码不正确!', -1);
        }
    ...
    public function checkParam()
    {
        ...
        } elseif ((empty($this->dparam['uuid']) || empty($this->dparam['idfa'])) && (empty($this->dparam['bdid']))) {
            info('设备信息不完整', -1);
        }

        //检查手机号码
        if (empty($this->dparam['phone']) || !preg_match("/^1[34578]\d{9}$/", $this->dparam['phone'])) {
            info('非法手机号', -1);
        }
    }
