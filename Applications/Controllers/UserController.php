...
class UserController extends AppController
{
    ...
    public function checkbindMasters($uid, $sfuid)
    {
        try {
            //验证用户是否已经注册登录
            if (!$user = userBehaviorVerificationController::userRegistration($uid)) {
                E('您赶紧去注册登录吧');
            }
            //验证用户是否已经绑定过师傅了
            if (!empty($user['sfuid'])) {
            ...
            //验证要绑定的好友账号是否存在
            if (!$sf = userBehaviorVerificationController::userRegistration($sfuid)) {
                E('您填写的好友不存在');
            }
            if ($sf['objectId'] == $user['objectId']) {
                E('不允许绑定自己');
            }
            //验证该用户是否已经淘宝授权过 且该淘宝账号只被授权过一次
            if (!$taobaoInfo = userBehaviorVerificationController::queryTaoBaoAuthId($user['taobao_id'])) {
                E('请您先淘宝授权');
            }
            ...
            if (empty($sf['taobao_id'])) {
                E('您的好友可能还未允许淘宝授权暂时无法绑定好友');
            }
            if ($sf['taobao_id'] == $user['taobao_id']) {
                E('淘宝授权账号重复暂时无法绑定好友');
            }
    ...
    public function redMessage()
    {
        ...
        foreach ($info as $k => $v) {
            if ($v['status'] == 1) {    //未领取红包
                $msg_info['untake'][] = $v;
            } elseif ($v['status'] == 2) {    //已领取红包
                $msg_info['token'][] = $v;
            } elseif ($v['status'] == 3) {    //退单的失效红包
                $msg_info['fail'] = $v;
            }
        }
    ...
    {
        $params = $_REQUEST;
        if (!$params['phone'] || !$params['user_id']) {
            info('数据不完整', -1);
        }
        ...
        if (!empty($friend_exisit)) {
            info('亲,您已经被邀请过!', -1);
        }
        $add_data = ['phone'=>$params['phone'],'sfuid'=>$params['user_id']];
        if (M('friend_log')->add($add_data, true)) {
            info('您已成功被邀请!', 1);
        }
    }
