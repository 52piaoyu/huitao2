...
class UserRecordController extends AppController
{
    ...
    {
        if (!isset($uid) || !isset($numid) || !isset($system)) {
            info('数据不完整Record', -1);
        }
    ...
    public function usersBrowseMerchandiseRecords($uid, $numid)
    {
        ...
        //去重重复的数据
        if (isset($usersBrowseMerchandiseRecords[$numid])) {
            unset($usersBrowseMerchandiseRecords[$numid]);
        }
    ...
    {
        if (!isset($uid) || !isset($system) || !isset($sharetype)) {
            info('数据不完整', -1);
        }
        $this->type = 'share';
        $this->uid = $uid;
    ...
    {
        if (!isset($uid) || !isset($content) || !isset($system)) {
            info('数据不完整', -1);
        }
