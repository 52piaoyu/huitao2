...
class HtreportModel {
    ...
    //由用户的好友带来的毛利润 (刨除掉退单的)
    public function userFriendProfits($startTime, $endTime) {
        M()->query("SELECT sum(benifit) td_benifit , report_date FROM ngw_shopping_log WHERE uid IN(SELECT objectId FROM ngw_uid WHERE sfuid IN(".(self::$uid).")) AND order_id NOT IN( SELECT order_id FROM ngw_shopping_log WHERE order_status = 5) AND report_date BETWEEN '{$startTime}' AND '{$endTime}' GROUP BY report_date", 'all');
    }
    //由用户给我们带来的毛利润(刨除掉退单的)
    public function userMaoProfits($startTime, $endTime) {
        return M()->query("SELECT sum(benifit) benifit , report_date FROM ngw_shopping_log WHERE uid IN( SELECT objectId FROM ngw_uid WHERE sfuid IN(".(self::$uid).")) AND order_id NOT IN( SELECT order_id FROM ngw_shopping_log WHERE order_status = 5) AND report_date BETWEEN '{$startTime}' AND '{$endTime}' GROUP BY report_date", 'all');
    }
    //拿订单号计算出该商品我们获得的净利润
